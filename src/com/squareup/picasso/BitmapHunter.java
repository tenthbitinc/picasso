...
abstract class BitmapHunter implements Runnable {
    ...
    protected static final int ASSET_PREFIX_LENGTH = (SCHEME_FILE + ":///" + ANDROID_ASSET + "/")
            .length();
    ...
    static BitmapHunter forRequest(Context context, Picasso picasso, Dispatcher dispatcher,
        ...
        if (SCHEME_CONTENT.equals(scheme)) {
            if (Contacts.CONTENT_URI.getHost().equals(uri.getHost()) //
                ...
                return new ContactsPhotoBitmapHunter(context, picasso, dispatcher, cache, stats,
                        action);
            } else {
                return new ContentProviderBitmapHunter(context, picasso, dispatcher, cache, stats,
                        action);
            }
        ...
        for (int i = 0, count = transformations.size(); i < count; i++) {
            ...
            if (newResult == null) {
                StringBuilder builder = new StringBuilder()
                        //
                        .append("Transformation ").append(transformation.key())
                        .append(" returned null after ").append(i)
    ...
    static Bitmap transformResult(Request data, Bitmap result, int exifRotation) {
        ...
        if (data.needsMatrixTransform()) {
            ...
            if (data.centerCrop) {
                ...
                if (data.onlyResizeDown == false || scale <= 1) {
                ...
                if (data.onlyResizeDown == false || sx <= 1 && sy <= 1) {
        ...
        Bitmap newResult = Bitmap.createBitmap(result, drawX, drawY, drawWidth, drawHeight, matrix,
                true);
        if (newResult != result) {
            result.recycle();
            result = newResult;
